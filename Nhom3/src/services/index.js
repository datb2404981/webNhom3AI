...
// DESCRIPTION:
// Nhận file Buffer từ multer và mimeType, gọi Gemini AI để nhận diện biển báo.
// Returns: { content, status }
//   - status 200: content là JSON string { Sign_id: "..." }
//   - status 429: content là chuỗi lỗi quota
//   - status 404: content là chuỗi lỗi khác

const call_ai = async (fileBuffer, mimeType) => {
  try {
    // Chuyển Buffer sang Base64 (Node.js native - không dùng Web API)
    ...
    const response = await AI.models.generateContent({
      ...
      contents: [user_prompt, image_prompt],
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            Sign_id: {
              type: Type.STRING,
              description:
                "Mã số biển báo giao thông trong hình theo luật đường bộ Việt Nam",
            },
          },
          required: ["Sign_id"],
        },
